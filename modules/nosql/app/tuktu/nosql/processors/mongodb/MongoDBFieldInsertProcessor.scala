...
import tuktu.api.utils.{ MapToJsObject, AnyToJsValue, evaluateTuktuString }
...
class MongoDBFieldInsertProcessor(resultName: String) extends BaseProcessor(resultName) {
    ...
    override def processor(): Enumeratee[DataPacket, DataPacket] = Enumeratee.mapM((data: DataPacket) => {
        ...
        (
            ...
            field match { 
                case None 		    => MapToJsObject( datum, true )
                case Some( key )  => AnyToJsValue( datum( key ), true ).as[JsObject]
            })
